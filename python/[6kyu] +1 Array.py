...
def up_array(arr):
    converted = "".join([str(num) for num in arr])

    if len(arr) == 0 or (len(converted) != len(arr)):
        return None

    final_num = list(str(int(converted) + 1))

    if len(converted) > len(final_num):
        final_num.insert(0, 0)

    return [int(num) for num in final_num]

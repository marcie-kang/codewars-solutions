...
def closest(strng):
    ...
    copied = strng.strip().split(" ")

    info_list = []
    for idx, digit in enumerate(copied):
        weight = sum(int(d) for d in digit)
        info_list.append([weight, idx, int(digit)])

    min_diff = float('inf')
    for i in range(len(info_list)):
        for j in range(i + 1, len(info_list)):
            diff = abs(info_list[i][0] - info_list[j][0])
            min_diff = min(min_diff, diff)

    candidates = []
    for i in range(len(info_list)):
        for j in range(i + 1, len(info_list)):
            if abs(info_list[i][0] - info_list[j][0]) == min_diff:
                pair = [info_list[i], info_list[j]]

                weight_sum = info_list[i][0] + info_list[j][0]
                min_weight = min(info_list[i][0], info_list[j][0])
                candidates.append((weight_sum, min_weight, i, j, pair))

    candidates.sort(key=lambda x: (x[0], x[1], x[2], x[3]))
    result_pair = candidates[0][4]
    result_pair.sort(key=lambda x: (x[0], x[1]))

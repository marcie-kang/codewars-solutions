...
def moving_shift(s, shift):
    ...
    for i, char in enumerate(s):
        ...
        new_char_code = (ord(char) - base + current_shift) % 26 + base
        coded_chars.append(chr(new_char_code))

    coded_s = "".join(coded_chars)
    L = len(coded_s)

    parts = []
    part_len = math.ceil(L / 5)

    start_index = 0
    remaining_length = L

    for _ in range(4):
        current_len = min(part_len, remaining_length)
        part = coded_s[start_index: start_index + current_len]
        parts.append(part)

        start_index += current_len
        remaining_length -= current_len

        if remaining_length <= 0:
            break

    if start_index < L:
        parts.append(coded_s[start_index:])
...
def demoving_shift(s, shift):
    ...
    for i, char in enumerate(coded_s):
        current_shift = (shift + i) % 26

        if 'a' <= char <= 'z':
            base = ord('a')
            new_char_code = (ord(char) - base - current_shift + 26) % 26 + base

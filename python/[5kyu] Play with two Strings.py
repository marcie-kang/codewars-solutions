...
def convert_word(a, b):
    copied_b = b
    checked = []

    for alp_a in a:
        if alp_a not in checked:
            count = Counter(a.lower())
            new_str = ""

            for alp_b in b:
                if count[alp_b.lower()] % 2 != 0:
                    if alp_b.isupper():
                        new_str += alp_b.lower()
                    else:
                        new_str += alp_b.upper()
            ...
            copied_b = new_str
            checked.append(alp_a)

    return copied_b

def work_on_strings(a, b):
    converted_a = convert_word(a, b)
    converted_b = convert_word(b, a)

    return converted_b + converted_a
